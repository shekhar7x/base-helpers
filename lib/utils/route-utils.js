...
routeUtils.setRequiredKeys = (baseBodyJoi, requiredKeys = []) => {
    return Object.assign(
        {},
        baseBodyJoi,
        requiredKeys.reduce((acc, key) => {
            acc[key] = baseBodyJoi[key].required()
            return acc;
        }, {})
    )
}
        ...
        handler(request, response, next)
            .then((result) => {
                ...
                if ((route.handleResponseFormat !== false) && !(result || {}).statusCode && !(result || {}).message) {
                    result = httpUtils.getSuccessResponse(result, (result || {}).message, (result || {}).statusCode);
                }
...
let setMongooseId = () => {
    let joiObject = Joi.string().regex(/^[a-f\d]{24}$/i);
...
routeUtils.validation = {
    ...
    optionalString: Joi.string().allow(null, '').optional(),
...
let getMiddleWaresArray = (route) => {
    ...
    let authorizor = route.authorize;
