...
const customJoi = Joi.extend((joi) => ({
    base: joi.array(),
    name: 'stringArray',
    coerce: (value, state, options) => (value.split ? value.split(',').replace(/^,+|,+$/mg, '') : value)
}))
...
routeUtils.validation = {
    queryStringArrayEnums: (enums) => customJoi.stringArray().items(Joi.string().valid(enums)).single(),
