...
class CrudApi {
    ...
    getIncludeQueryParams() {
        let includeParams = _.get(this, 'controller.module.includeEnums');
        if (includeParams instanceof Array && includeParams.length) {
            return { include: validation.queryStringArrayEnums(includeParams) };
        }
        return {};
    }
    ...
    getFindByIdApi(description) {
        let include = this.getIncludeQueryParams();
        return this.getDefaultApi('GET', true, {}, include, 'findById', this.controller.findById.bind(this.controller), description);
    }
