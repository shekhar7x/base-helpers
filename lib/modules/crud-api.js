...
class CrudApi {
    ...
    getDefaultApi(method = 'GET', idRequired = false, body, query, action, handler, description, customMethod) {
        action = S(action || '').humanize().s
        let defaultDescription = `Perform ${action} action on ${this.name}`;
        let customMethodString = customMethod ? `/${customMethod}` : '';
        let path = (idRequired ? '/:id' : '') + customMethodString
        let api = {
            method,
            path: path || '/',
            schema: {
